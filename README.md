...
KBL 공식 경기 기록을 이용해 2013-14~2024-25 정규리그 12개 시즌, 3,183경기를 수집하고,
팀 관점 데이터와 LSTM/GRU용 시퀀스를 생성합니다. 2019-20은 리그 중단으로 213경기이며,
나머지 시즌은 각 270경기입니다.
...
- `games_raw.csv`: 경기당 1행(총 3,183행)
- `team_games.csv`: 팀 관점 경기당 2행(총 6,366행)
...
원본 KBL API 응답은 GitHub Release의 `kbl_raw_data_2013-14_2024-25.zip`에서 내려받아
...
3,183경기의 승리팀과 패배팀을 대응쌍으로 구성해 분포, 승패 차이 검정, 효과크기, 상관관계,
...
12개 시즌 사이의 로스터 변화 영향을 비교할 수 있도록 두 종류의 시퀀스를 제공합니다.
...
구단 코드가 바뀐 전자랜드→한국가스공사(`65→64`)와 오리온→캐롯→소노(`30→73→66`)는
분석용 `franchise_id`로 같은 계보를 연결합니다. 원래 `team_code`도 함께 보존하므로 시점별 구단 코드를
확인할 수 있습니다.

시간 순 분할은 시즌 중간을 자르지 않습니다.

- 학습: 2013-14~2020-21(2,103경기)
- 검증: 2021-22~2022-23(540경기)
- 테스트: 2023-24~2024-25(540경기)
