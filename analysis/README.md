...
`scripts/run_statistical_analysis.py`를 실행하면 KBL 2013-14~2024-25 정규리그 3,183경기 데이터를 이용해
분포, 승패 차이, 효과크기, 상관관계, VIF, PCA, 팀별 이동평균 분석을 생성합니다.

2019-20은 리그 중단으로 213경기이며 나머지 11개 시즌은 각 270경기입니다. 구단 코드 변경은
`franchise_id`로 연결해 장기 추이를 표시하되, 이동평균은 시즌 시작마다 초기화합니다.
